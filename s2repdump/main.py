...
def setup_info(s2rep: S2Replay):
    ...
    info.map_info.cache_handles = [
        *map(
            # lambda x: '%s.%s' % (b2a_hex(x[16:]).decode(), x[0:4].decode('ascii')),
            lambda x: '%s' % (b2a_hex(x[16:]).decode()),
            s2rep.details['m_cacheHandles'])
    ]
    info.map_info.author_handle = s2rep.init_data['m_syncLobbyState'][
        'm_gameDescription']['m_mapAuthorName'].decode() or None
...
def setup_participants(s2rep: S2Replay):
    ...
    for key, dp_entry in enumerate(s2rep.details['m_playerList']):
        ...
        if dp_entry['m_control'] == EPlayerControl.HUMAN:
            if dp_entry['m_toon']['m_region']:
                pinfo.handle = '%d-S2-%d-%d' % (dp_entry['m_toon']['m_region'],
                                                dp_entry['m_toon']['m_realm'],
                                                dp_entry['m_toon']['m_id'])
            else:
                pinfo.handle = None
        ...
        pinfo.color = PlayerColor(mcol['m_r'], mcol['m_g'], mcol['m_b'],
                                  mcol['m_a'])
        ...
        if s2rep.features.working_slots:
            if dp_entry['m_workingSetSlotId'] is None:
                ...
                logging.warning('"%s" has no working slot assigned' %
                                (pinfo.name))
                pinfo.uid = plist[-2].uid + 1 if len(plist) > 1 else 0
                continue

            for slot_index, sl_slot in enumerate(
                    s2rep.init_data['m_syncLobbyState']['m_lobbyState']
                ['m_slots']):
                if dp_entry['m_workingSetSlotId'] != sl_slot[
                        'm_workingSetSlotId']:
                    continue
            ...
            sl_slot = s2rep.init_data['m_syncLobbyState']['m_lobbyState'][
                'm_slots'][next_slot]
            pinfo.working_slot = next_slot
            pinfo.uid = sl_slot['m_userId']

        if dp_entry[
                'm_observe'] == EObserve.NONE or not s2rep.features.working_slots:
    ...
    if s2rep.features.tracker_player_pid:
        while True:
            ...
            if p is None:
                logging.warning(
                    'Failed to match a slot_id of %d with a pid of %d' %
                    (ev['m_slotId'], ev['m_playerId']))
                continue
            p.pid = ev['m_playerId']
    ...
    for ev in s2rep.gameevents:
        if ev['_event'] in BANK_EVENTS:
            ...
            if ev['_event'] == 'NNet.Game.SBankFileEvent':
                ...
                banks[puid].append(
                    GameBankMeta(ev['m_name'].decode('ascii'), player))
...
def main(args):
            ...
            def append_prop(val, key_path: str = ''):
                if hasattr(val, 'props'):
                    for sub_key in val.props:
                        append_prop(val[sub_key],
                                    f'{key_path}.{sub_key}'.lstrip('.'))
    ...
    if 'chat' in args.decode and s2rep.messageevents:
        ...
        for ev in s2rep.messageevents:
            ...
            if '_userid' in ev:
                participant = s2rep.participants.get_player_by_uid(
                    ev['_userid']['m_userId'])
            elif '_playerid' in ev:
                participant = s2rep.participants.get_player_by_pid(
                    ev['_playerid']['m_playerId'])
            else:
            ...
            if args.json:
                sections['chat'].append({
                    ...
                    participant.uid,
                    'recipient':
                    EMessageRecipient[ev['m_recipient']].lower(),
                    'message':
                    msg,
                })
            else:
                print('%s | %06s | %-s: %s' %
                      ('%d:%02d:%02d' %
                       ...
                       EMessageRecipient[ev['m_recipient']], participant.name,
                       msg))
        if not args.json:
            print()
    ...
    if args.bank_rebuild:
        ...
        if not args.json and not args.force and os.path.isdir(
                args.out) and len(os.listdir(args.out)) > 0:
            logging.error(
                'Specified output directory "%s" already exists and is not empty, aborting..'
                % (args.out))
        else:
            for gbmeta in s2rep.banks:
                ...
                logging.info(
                    f'Rebuilding "{gbmeta.name}.SC2Bank" for player {pname} ..'
                )
                bank_store = GameBankStorage()
                bank_store.rebuild_from_meta(gbmeta)

                expected_signature = bank_store.signature()
                computed_signature = bank_store.compute_signature(
                    s2rep.info.map_info.author_handle, gbmeta.player.handle)
                if expected_signature is not None and expected_signature != computed_signature:
                    logging.warning(
                        ...
                        pname, bank_store.name, expected_signature,
                        computed_signature)
                ...
                if args.json:
                    sections['sc2banks'].append({
                        ...
                        computed_signature,
                        'filename':
                        bank_store.filename(s2rep.info.map_info.author_handle,
                                            gbmeta.player.handle),
                        'content':
                        bank_store.tostring(not args.json_compact),
                    })
                else:
                    filename = bank_store.write_sc2bank(
                        args.out, True, s2rep.info.map_info.author_handle,
                        f"{gbmeta.player.handle}_{gbmeta.player.name.replace(' ', '_').replace('/', '_')}"
                    )
